...
2. **Multi-OS Fact Branching:** Separates nodes by operating system (`linux_nodes` vs `windows_nodes`) so Ansible roles autonomously branch directory paths (`/opt/tm-home` or `~/.local/share/` vs `C:\tm-home`), credential management (`0400` vs NTFS ACLs), and service managers (`systemd --user` vs Windows Service).
