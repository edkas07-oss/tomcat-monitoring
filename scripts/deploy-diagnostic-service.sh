...
main() {
    ...
    "${CONTAINER_ENGINE}" cp postfix-relay:/etc/postfix/tls/server.crt "${TLS_DIR}/postfix-ca.crt" 2>/dev/null || true
    ...
    local run_args=(
        ...
        --volume "${CONFIG_FILE}:/run/tomcat-diagnostic/application.json${vol_ro_z}"
        --volume "${TARGETS_FILE}:/run/tomcat-diagnostic/config/targets.json${vol_ro_z}"
        --volume "${SECRETS_DIR}/bearer-token:/run/tomcat-diagnostic/secrets/bearer-token${vol_ro_z}"
        --volume "${SECRETS_DIR}/smtp-username:/run/tomcat-diagnostic/secrets/smtp-username${vol_ro_z}"
        --volume "${SECRETS_DIR}/smtp-password:/run/tomcat-diagnostic/secrets/smtp-password${vol_ro_z}"
        --volume "${TLS_DIR}/server.crt:/run/tomcat-diagnostic/tls/server.crt${vol_ro_z}"
        --volume "${TLS_DIR}/server.key:/run/tomcat-diagnostic/tls/server.key${vol_ro_z}"
        --volume "${TLS_DIR}/postfix-ca.crt:/run/tomcat-diagnostic/tls/postfix-ca.crt${vol_ro_z}"
        --volume "${SPOOL_DIR}:/run/tomcat-diagnostic/spool${vol_ro_z}"
        --volume "${LOG_VOLUME}:/run/tomcat-diagnostic/logs${vol_ro_z}"
        --volume "${DATA_VOLUME}:/var/lib/tomcat-diagnostic${vol_z}"
    )
    if [[ -n "${userns_flag}" ]]; then
        run_args=("${userns_flag}" "${run_args[@]}")
    fi
    ...
    sleep 6
    if [[ "$("${CONTAINER_ENGINE}" inspect --format '{{.State.Status}}' "${CONTAINER_NAME}")" == "running" ]]; then
        echo "Diagnostic Service is running."
        trap - ERR
        if container_exists "${ROLLBACK_NAME}"; then
            "${CONTAINER_ENGINE}" rm -f "${ROLLBACK_NAME}" >/dev/null 2>&1 || true
        fi
    else
        rollback_on_failure
