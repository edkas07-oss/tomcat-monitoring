...
| Topology Profile | Target Components | Architecture & Purpose | Deployment Flexibility & CLI Command | Test Status |
| :--- | :--- | :--- | :--- | :---: |
| **`all_in_one`** *(Default)* | Tomcat, Prometheus, Alertmanager, Diagnostic Service, Postfix, Mailpit, tm-agent | Single-server co-located deployment | **Instant Single-Node Setup**<br/>*(Default — no extra flag)* | ✅ **Verified & Tested** |
| **`monitoring_node`** | Tomcat, Telegraf, tm-agent | Monitored application server shipping metrics/events | **Lightweight Edge Agent Scaling**<br/>`-e "deploy_topology=monitoring_node"` | 🧪 *Experimental / Testing* |
| **`central_hub`** | Prometheus, Alertmanager, Diagnostic Service, Postfix, Mailpit | Dedicated monitoring server aggregating multi-node fleet | **Centralized Multi-Fleet Management**<br/>`-e "deploy_topology=central_hub"` | 🧪 *Experimental / Testing* |
| **`custom`** | User-defined list in `selected_components` | Tailored component allocation | **100% Granular Component Selection**<br/>`-e "deploy_topology=custom" -e 'selected_components=["prometheus","alertmanager"]'` | 🧪 *Experimental / Testing* |

> [!CAUTION]
> **Production Notice on Distributed Topologies:**  
> The distributed multi-node topology profiles (`monitoring_node`, `central_hub`, `custom`) are currently in an active testing phase. For production environments, it is recommended to use the fully verified default **`all_in_one`** topology, or validate distributed setups in staging before production deployment.
